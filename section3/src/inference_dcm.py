...
def get_predicted_volumes(pred):
    ...
    volume_ant=np.count_nonzero(pred==1)
    volume_post=np.count_nonzero(pred==1)
...
def create_report(inference, header, orig_vol, pred_vol):
    ...
    #centering around the mid axial point
    slice_nums = [orig_vol.shape[2]//3, orig_vol.shape[2]//2, orig_vol.shape[2]*2//3] # is there a better choice?
    ...
    #let's take the slice in middle of axial
    slice_orig=orig_vol[slice_nums[0],:,:]
    slice_pred=pred_vol[slice_nums[0],:,:]
    nd_img = np.flip((slice_orig/np.max(orig_vol))*0xff).T.astype(np.uint8)
    nl_img = np.flip((slice_pred/np.max(pred_vol))*0xff).T.astype(np.uint8)
    # This is how you create a PIL image from numpy array
    dim_width=min(nd_img.shape[0],nl_img.shape[0])
    dim_height=min(nd_img.shape[1],nl_img.shape[1])
    pil_i = Image.fromarray(nd_img, mode="L").convert("RGBA").resize(nd_img.shape)
    pil_l = Image.fromarray(nl_img, mode="L").convert("RGBA").resize(nl_img.shape)
    ...
    draw.text((10, 555), "Label Slice", (255, 255, 255), font=header_font)
    pimg.paste(pil_l, box=(10, 560))
    return pimg
...
def save_report_as_dcm(header, report, path):
    ...
    print("Debug save_report path:",path,"out:",out)
...
def get_series_for_inference(path):
    ...
    print("HippoCrop Series contains:",len(series_for_inference)," dcm slices")
    # Check if there are more than one series (using set comprehension).
    if len(series_for_inference)==0:
        print("Error No HyppoCrop Series found")
        return []
...
def get_dcm(path):
    list_dcm_files= [pydicom.dcmread(os.path.join(path, f)) for f in os.listdir(path)]
    print(list_dcm_files[0])
...
if __name__ == "__main__":
    ...
    # routed studies
    print("Main...")
    print("arg v1:",sys.argv[1])
    #path="../data/Study1/13_HCropVolume"
    #path="../data/Study1/st_1.3.6.1.4.1.14519.5.2.1.4429.7055.290332099546120305394775536058"
    path=sys.argv[1]
    #get_dcm(path)
    if len(sys.argv) != 2:
    ...
    print("Subdirs type len subdirs:\n",type(subdirs),len(subdirs),subdirs)
    ...
    print("New subdirs:",selsubdirs)
    #This takes the directories in order of last modification and select the first        
    ...
    os_command(f"sudo storescu 127.0.0.1 4242 -v -aec HIPPOAI +r +sd {report_save_path}")
