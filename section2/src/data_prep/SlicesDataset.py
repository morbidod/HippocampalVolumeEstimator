...
class SlicesDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # <YOUR CODE GOES HERE>
        image_volume=self.data[slc[0]]['image']
        label_volume=self.data[slc[0]]['seg']
        image_slice=image_volume[slc[1],:,:]
        label_slice=label_volume[slc[1],:,:]
        sample['image']=torch.from_numpy(image_slice).type(torch.cuda.FloatTensor).unsqueeze(0)
        sample['seg']=torch.from_numpy(label_slice).type(torch.cuda.LongTensor).unsqueeze(0)
        return sample
