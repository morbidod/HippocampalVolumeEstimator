...
from utils.volume_stats import Dice3d, Jaccard3d,check_value
...
class UNetExperiment:
    ...
    def train(self):
        ...
        for i, batch in enumerate(self.train_loader):
            ...
            if(i==0):
                print("data shape:",data.shape,"prediction:",prediction.shape)    
            ...
            # ANSWER:
            #prediction is a tensor toarch of size (8,3,64,64)
            # 8 is the batch size
            # 3 is the number of Channel output should be background, anterior label and posterior label since the label have these 2 values
            #64,64 is the tensor predicted label (the images with dimension inferior to 64 have already been padded with 0s
            loss.backward()
            self.optimizer.step()
    ...
    def run_test(self):
        ...
        for i, x in enumerate(self.test_data):
            ...
            print("Processing:",i,"volume data:",x['image'].shape," file:",x['filename'])   
            check_value(pred_label)
            check_value(x['seg'])
